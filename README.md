...
客户端和服务端关于store的代码有不少相同的地方，可将相同代码进行拆分处理，见[分支daily/0.0.9]()。

注意：上述分支代码存在一个大坑，即store/index.js中代码创建的是单例store。对于server端的render方法来说，当用户访问页面的时候，所有的用户访问的是同一个store，也就是说所有的用户用的是同样数据，正常情况下应该是不同的用户访问的数据不一样。

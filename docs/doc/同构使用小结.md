...
react-router-dom提供的Redirect方法只能进行客户端重定向，不能进行服务端重定向。要想实现服务端301重定向，此时可以利用react-router-config。react-router-config在定义路由的时候，当组件内部出现Redirect方法的时候，它会自动帮我们处理context， 代码修改见分支[daily/0.2.1]()

...
假设一个页面要加载A、B、C、D四个组件，这四个组件都需要在服务端加载数据。假设A组件加载数据错误，B、C、D组件有以下几种情况：
...
针对这种情况，可以额外封装一层Promise，让对应加载的组件无论成功或者失败，都使得最外层的Promise执行成功。对于加载失败的组件，则不显示数据，代码修改见分支[daily/0.2.2]()

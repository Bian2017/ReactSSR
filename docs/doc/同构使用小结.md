...
服务端将页面必要的redux数据存放到window.context的过程，称之为注水；
客户端渲染的时候将数据从window.context取出来使用的过程，称之为脱水；

注意：并不能因为服务端渲染已经有数据了，就代表componentDidMount中获取数据的过程就不需要了。因为服务端渲染只发生在第一次进入页面时，当第一次进入的页面是Login页面，此时通过**浏览器路由**进入Home页面则依旧无数据。
